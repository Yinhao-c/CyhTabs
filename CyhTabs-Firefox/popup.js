  ...
  function loadGroups(filterText = '') {
    chrome.storage.sync.get(['tabGroups'], (result) => {
      ...
      if (filteredGroups.length === 0) {
        ...
        `;
        groupList.appendChild(emptyStateDiv);
      ...
      filteredGroups.forEach((group, index) => {
        ...
        groupBadgeSpan.className = "group-badge";
        groupBadgeSpan.textContent = `${group.tabs.length} tabs`;
        groupHeader.appendChild(groupNameSpan);
        groupHeader.appendChild(groupBadgeSpan);
        ...
        openBtn.dataset.index = actualIndex;
        openBtn.innerHTML = `<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M2 7L6 11L12 3" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg> Open`;

        const renameBtn = document.createElement("button");
        renameBtn.className = "action-btn rename-btn";
        renameBtn.dataset.index = actualIndex;
        renameBtn.innerHTML = `<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M10 2L12 4L5 11H3V9L10 2Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg> Rename`;

        const deleteBtn = document.createElement("button");
        deleteBtn.className = "action-btn delete-btn";
        deleteBtn.dataset.index = actualIndex;
        deleteBtn.innerHTML = `<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 3L11 11M11 3L3 11" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg> Delete`;
    ...
    if (target.classList.contains('open-btn')) {
      ...
      chrome.storage.sync.get(['tabGroups'], (result) => {
        ...
        groupToEdit.tabs.forEach((tab, tabIndex) => {
          ...
          const input = document.createElement("input");
          input.type = "checkbox";
          input.id = `tab-${tabIndex}`;
          input.checked = true;
          const label = document.createElement("label");
          label.htmlFor = `tab-${tabIndex}`;
          label.textContent = tab.title || tab.url;
          checkboxDiv.appendChild(input);
          checkboxDiv.appendChild(label);
          tabSelectionList.appendChild(checkboxDiv);
